...
#' @param flowcell_capacities A vector of flow capacities
#' @param flowcell_costs A vector of flow costs
...
FastQDesign <-
  function(
    ...
    if (budget < min(library_costs + flowcell_costs)) {
      stop(
        strwrap(
          "\033[31mError: Budget is smaller than\n      minimum cost(`flowcell_costs` + `library_costs`), please\n      increase it!\033[0m"
        )
      )
    }
    ...
    model_scam <- scam::scam(
      power ~ s(N, k = 10, bs = "mpi") +
        s(R, k = 10, bs = "mpi"),
      data = df_power
    )
    df_flow_list <- list()
    for (i in seq_along(flowcell_capacities)) {
      ...
      df_flow_list[[i]] <-
        data.frame(
          N = N,
          R = R,
          power = predict(model_scam, newdata = data.frame(N = N, R = R))
        ) %>%
        dplyr::filter(N < max_N, R < max_R) %>%
        dplyr::mutate(flow_cell = i, cost = library_costs + flowcell_costs[i])
    }
    df_flow <-
      do.call(rbind, df_flow_list) %>%
    ...
    capacties_afforable <-
      (budget - library_costs) /
      min(flowcell_costs / original_capacities) *
      reads_valid_rate
    curves_flow_shared <-
    ...
    ind_available_design <- df_flow %>%
      dplyr::filter(power >= power_threshold, cost <= budget)
    ...
    ind_optimal_power <-
      ind_available_design %>%
      dplyr::arrange(dplyr::desc(power)) %>%
      dplyr::slice_head() %>%
      dplyr::mutate(type = "similarity")
    ind_optimal_cost <-
      ind_available_design %>%
    ...
    colors_ind <-
      RColorBrewer::brewer.pal(
        n = length(flowcell_capacities) +
          3,
        name = "Set1"
      )[seq_len(1 + length(flowcell_capacities))]
    names(colors_ind) <- c(converted_capacities, "Budget")
    type_values <- c(similarity = 8, cost = 9)
    p_design_ind <- ggplot2::ggplot(df_flow, ggplot2::aes(N, R)) +
      ggplot2::geom_point(shape = 21, size = 3, ggplot2::aes(fill = feasible)) +
      ...
      ggplot2::scale_y_continuous(
        labels = scales::label_number(suffix = " K", scale = 0.001),
      ...
      ggplot2::theme_bw()
    p_power_cost_ind <- ggplot2::ggplot(df_flow, ggplot2::aes(cost, power)) +
      ggplot2::geom_point(shape = 21, size = 3, ggplot2::aes(fill = feasible)) +
      ggplot2::scale_fill_manual(
        values = c("grey30", "grey70"),
        breaks = c(TRUE, FALSE),
      ...
      ggplot2::labs(x = "Cost", y = "Similarity", fill = "Feasibility") +
    ...
    shared_flow_cells_index <-
      which.min(flowcell_costs / flowcell_capacities)
    flow_cell_used <- converted_capacities[shared_flow_cells_index]
    df_power_shared$cost <-
      library_costs +
      min(flowcell_costs / flowcell_capacities) *
        df_power_shared$R *
        df_power_shared$N
    df_power_shared$flow_cell <- flow_cell_used
    df_power_shared$feasible <- ifelse(
      df_power_shared$cost <= budget,
      TRUE,
      FALSE
    )
    ...
    p_design_share <-
      ggplot2::ggplot(df_power_shared, ggplot2::aes(N, R)) +
      ggplot2::geom_point(shape = 21, size = 3, ggplot2::aes(fill = feasible)) +
      ggplot2::scale_fill_manual(
        values = c("grey30", "grey70"),
        breaks = c(TRUE, FALSE),
      ...
      ggplot2::theme_bw()
    p_power_cost_share <-
      ggplot2::ggplot(df_power_shared, ggplot2::aes(cost, power)) +
      ggplot2::geom_point(shape = 21, size = 3, ggplot2::aes(fill = feasible)) +
      ggplot2::scale_fill_manual(
        values = c("grey30", "grey70"),
        breaks = c(TRUE, FALSE),
