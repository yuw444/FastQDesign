---
title: "Generating Power Metadata with fastF"
author: "FastQDesign Team"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Generating Power Metadata with fastF}
  %\VignetteEngine{rmarkdown::html}
  %\VignetteEncoding{UTF-8}
---

This vignette demonstrates how to generate `df_power` metadata for experiment design using the `FastQDesign-fastF` tool and FastQDesign framework. The `df_power` data frame contains power/similarity metrics across a grid of cell proportions and read depths.
...
2. **Generate downsampled UMI matrices** - Using `fastF_bam2db()` across a parameter grid
3. **Compare with reference** - Using `SamplePrep()` and `SampleMatch()`
...
This vignette requires BAM files to generate downsampled UMI matrices. The package includes a small test dataset, but generating power metadata requires the full AI/BM BAM files from Zenodo.
...
**Note:** The package `reference_list.rds` contains pre-processed data for the sample-prep vignette, not the full BAM-to-matrix data needed here.

### Full Dataset (Zenodo)

For generating real power metadata, download the full AI/BM BAM files:

**BAM Files:** [https://zenodo.org/records/19073177](https://zenodo.org/records/19073177)
...
**Pre-processed Seurat Objects:** [https://zenodo.org/records/19072084](https://zenodo.org/records/19072084)
...
The BAM files contain Cell Ranger-aligned reads with CB (cell barcode) and UB (UMI) tags. After downloading, use the Cell Ranger output directory for feature and barcode files:
...
Alternatively, extract barcodes from the reference list downloaded from Zenodo:
...
For more details on data preparation, see the [External Data Sources](../data-sources/index.html) vignette.
...
```{r prepare_reference, eval=FALSE}
...
```{r generate_downsamples, eval=FALSE}
...
Load each downsampled matrix, compare with reference, and compute similarity metrics:
...
Average across replicates and convert to the format expected by `FastQDesign()`:

```{r aggregate_results, eval=FALSE}
...
Once you have the `df_power` data frame, you can use it with `FastQDesign()`:

```{r experiment_design, eval=FALSE}
...
- The grid uses rates (0-1) which are converted to absolute values based on reference
- 5 replicates (seeds) reduce computation while maintaining statistical power
- Similarity metrics include: ARI, Jaccard Index (cluster genes), Jaccard Index (condition genes), Kendall correlation (pseudotime), Kendall correlation (3D embedding)
