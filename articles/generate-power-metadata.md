...
This vignette requires BAM files to generate downsampled UMI matrices.
The package includes a small test dataset, but generating power metadata
requires the full AI/BM BAM files from Zenodo.

### Package Test Data

For quick testing, the package includes small test files:

``` r

# Test data paths (small files for learning the workflow)
...
**Note:** The package `reference_list.rds` contains pre-processed data
for the sample-prep vignette, not the full BAM-to-matrix data needed
here.

### Full Dataset (Zenodo)

For generating real power metadata, download the full AI/BM BAM files:
...
and UB (UMI) tags. After downloading, use the Cell Ranger output
directory for feature and barcode files:

``` r

# If you have Cell Ranger output, use the filtered_feature_bc_matrix directory
REF_AI <- "/path/to/P1_AI/outs/filtered_feature_bc_matrix"
REF_BM <- "/path/to/P1_BM/outs/filtered_feature_bc_matrix"
```

Alternatively, extract barcodes from the reference list downloaded from
Zenodo:
...
# Extract barcodes for AI and BM samples
ai_cells <- grep("^ai", colnames(ref_list$Seurat), value = TRUE)
bm_cells <- grep("^bm", colnames(ref_list$Seurat), value = TRUE)

writeLines(ai_cells, "ai_barcodes.tsv")
writeLines(bm_cells, "bm_barcodes.tsv")
```
