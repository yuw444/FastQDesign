...
The package includes small test files for learning the workflow:

``` r

# Test data paths (small files for testing)
...
**Reference Data:** <https://zenodo.org/records/19072084>
...
and UB (UMI) tags. Use the Cell Ranger output directory for feature and
barcode files:

``` r

# Use the filtered_feature_bc_matrix directory from Cell Ranger output
REF_AI <- "/path/to/P1_AI/outs/filtered_feature_bc_matrix"
REF_BM <- "/path/to/P1_BM/outs/filtered_feature_bc_matrix"

# Extract barcodes from reference list downloaded from Zenodo
download.file(
